    ...
    public Boolean validateSizeChartContents(List<String> expectedTableHeaders,
        ...
        try {
            ...
            for (int i = 0; i < expectedTableHeaders.size(); i++) {
                String exthead = expectedTableHeaders.get(i);
                WebElement acthead = driver.findElement(By.xpath("//table//th[" + (i + 1) + "]"));
                if (!exthead.equals(acthead.getText()))
                    status = false;
            }

            for (int i = 0; i < expectedTableBody.size(); i++) {
                List<String> row = expectedTableBody.get(i);
                for (int j = 0; j < row.size(); j++) {
                    WebElement tdata = driver.findElement(
                            By.xpath("//table//tbody/tr[" + (i + 1) + "]/td[" + (j + 1) + "]"));
                    if (!row.get(j).equals(tdata.getText()))
                        status = false;
                }
            }
